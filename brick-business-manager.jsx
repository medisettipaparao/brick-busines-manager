...
export default function App() {
  ...
  // ── Computed stats ────────────────────────────────────────
  const bCost = b => b.clayUsed*800+b.sandUsed*500+b.coalUsed*12000+b.waterUsed*50;
  ...
  const Production = () => (
    <div>
      ...
      <div style={{...C.card,padding:0,overflow:"auto"}}>
        <table style={C.tbl}>
          <thead><tr>{["Batch","Date","Bricks","Clay","Sand","Coal","Water","Mat.Cost","Cost/1000","Status","Actions"].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead>
          <tbody>{data.production.map((b,i)=>{const mc=bCost(b);return(
            <tr key={b.id} style={{background:i%2===0?"rgba(255,255,255,0.013)":""}}>
              ...
              <td style={C.td}>{b.clayUsed}T</td><td style={C.td}>{b.sandUsed}T</td><td style={C.td}>{b.coalUsed}T</td><td style={C.td}>{b.waterUsed}kL</td>
